...
```bash
conda create -n sample_project python=3.9
...
```
This command will create your personal repository under `/Repos/<username>/sample_project`.
...
- To trigger the release pipeline, get the current version from the `sample_project/__init__.py` file and tag the current code version:
```
